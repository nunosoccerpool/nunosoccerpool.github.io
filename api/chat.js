module.exports = async function handler(req, res) {
  // CORS headers must be set FIRST, before anything else, so they always reach the browser
  ...
  // Safely parse body — req.body may be undefined if Vercel hasn't parsed it
  let messages;
  try {
    const body = typeof req.body === "string" ? JSON.parse(req.body) : req.body;
    messages = body?.messages;
    if (!messages || !Array.isArray(messages)) throw new Error("No messages array");
  } catch (e) {
    return res.status(400).json({ reply: "Bad request: " + e.message });
  }
...
`;
  ...
  try {
    ...
    return res.status(200).json({ reply: "Groq fetch error: " + e.message });
  }
